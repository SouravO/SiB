...
export default function UserDashboard() {
    ...
    if (cityId) {
      ...
      if (collegesResult.success) {
        const cityUniversities =
          (await getUniversitiesByCity(cityId)).data || [];
        const cityUnivIds = cityUniversities.map((u: any) => u.id);
        const filtered = (collegesResult.data || []).filter((c: any) =>
          cityUnivIds.includes(c.university_id),
        );
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      if (allCollegesResult.success)
        setAllColleges(allCollegesResult.data || []);
  ...
  const filteredAllCities = allCities.filter(
    (city) =>
      city.name.toLowerCase().includes(citySearchQuery.toLowerCase()) &&
  ...
  const filteredInstitutions = allColleges.filter(
    (college) =>
    ...
    <div className="p-4 sm:p-6 lg:p-10 max-w-7xl mx-auto space-y-8 sm:space-y-10">
      ...
      <section className="px-2 sm:px-0">
        ...
        <p className="text-sm sm:text-base text-gray-500 font-medium">
          Find your dream institution today.
        </p>
      </section>
                  ...
                  {filteredStatesForDisplay.map((state) => (
                    ...
                    >
                      ...
                      <div className="absolute bottom-0 left-0 w-full p-6 sm:p-8 text-white">
                        ...
                        <h3 className="text-2xl sm:text-3xl font-black tracking-tight mt-1 mb-4">
                          {state.name}
                        </h3>
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-gray-500 text-xs sm:text-sm font-medium">
                  Results for {cities.find((c) => c.id === selectedCity)?.name}
                </span>
              </div>
          ...
          {loadingColleges ? (
            ...
            <div className="py-10 sm:py-20 text-center">
              ...
              <p className="text-sm sm:text-base text-gray-500 max-w-sm mx-auto">
                We couldn't find any institutions matching your search in this
                location. Try adjusting your filters.
              </p>
            </div>
          )}
        </section>
            ...
            <div className="flex justify-between items-center p-5 sm:p-8 border-b border-gray-50 sticky top-0 bg-white z-10">
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div>
                  ...
                  <p className="text-[9px] sm:text-xs text-gray-400 font-medium tracking-wide uppercase">
                    Verified Profile
                  </p>
                </div>
              </div>
